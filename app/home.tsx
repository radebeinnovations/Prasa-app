import { ImageBackground, Platform, StyleSheet, Text, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
export default function Home() {
  ...
  const availableHeight = Platform.OS === 'web' ? Math.min(windowHeight, 926) : windowHeight;
  const heroHeight = Math.min(520, Math.max(350, Math.round(availableHeight * 0.55)));
...
const styles = StyleSheet.create({
  container: { flex: 1, minHeight: 0, overflow: 'hidden', backgroundColor: '#FFFFFF' },
  hero: { width: '100%', flexGrow: 0, flexShrink: 0, overflow: 'hidden', backgroundColor: '#1B3548' },
  heroImage: { transform: [{ scale: 1.1 }, { translateY: -18 }] },
  ...
  actionsPanel: { flex: 1, minHeight: 0, backgroundColor: '#FFFFFF', paddingHorizontal: 14, paddingTop: 24 },
